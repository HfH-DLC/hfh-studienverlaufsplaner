...
export default {
    ...
    actions: {
        init({ commit, dispatch }, { planerSlug, plan, modules, focusSelections, rules }) {
        ...
        async creditedModuleAgainstFocusSelection({ commit, dispatch }, { moduleId, focusSelectionId }) {
        ...
        async save({ state, commit, getters }) {
            try {
                ...
                await dataAdapter.saveFocusCredits(getters.creditedModulesByFocusSelection)
    ...
    getters: {
        creditedModulesByFocusSelection(state) {
            return state.focusSelections.reduce((acc, cur) => {
                ...
                acc[cur.id] = state.modules.filter(module => module.creditedAgainst == cur.id);
                return acc
            }, {})
        }
    }
